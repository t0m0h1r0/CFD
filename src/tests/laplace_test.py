...
class LaplaceTestConfiguration:
    ...
    def __init__(
        ...
        self.boundary_conditions = boundary_conditions or self._default_boundary_conditions()
        self.order = order
...
class LaplacianTestSuite:
    ...
    def _create_3d_grid(self, grid_size: int) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
        """3Dグリッドを生成"""
...
def main():
    ...
    test_suite = LaplacianTestSuite(
        test_functions=default_test_functions(),
        config=config
    )
