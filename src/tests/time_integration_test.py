        ...
        for case_name, case in self.test_cases.items():
            ...
            if case.exact_solution is not None:
                ...
                if exact_array.ndim == 1:
                    ...
                    ax1.plot(times, exact_array[:, 0], 'k--', label='Exact', linewidth=2)
            ...
            for scheme_name, scheme in self.schemes.items():
                ...
                if numerical_array.ndim == 1:
                    line1 = ax1.plot(times[:len(numerical_array)],
                                   numerical_array,
                                   label=f'{scheme_name} (solution)',
                                   color=colors[scheme_name])[0]
                else:
                    line1 = ax1.plot(times[:len(numerical_array)],
                                   numerical_array[:, 0],
                                   label=f'{scheme_name} (solution)',
                ...
                # 誤差のプロット（右軸）
                if case.exact_solution is not None:
                    error = self._compute_error(numerical_array, exact_array[:len(numerical_array)])
                    line2 = ax2.plot(times[:len(error)],
                                   error,
                                   '--',
