...
class CCDTestSuite:
    ...
    def create_grid_manager(
        cls, points: int = 64, dimension: float = 1.0
    ) -> GridManager:
        """
    ...
    def create_boundary_conditions(
        cls, bc_type: BCType = BCType.DIRICHLET
    ...
    def evaluate_derivative_accuracy(
        ...
        for points in points_range:
            ...
            ccd_solver = CCDCompactDifference(
                grid_manager=grid_manager, direction="x", boundary_conditions=bc
            )
            # 1D のグリッド生成
            ...
            rel_error = jnp.linalg.norm(computed - exact) / (
                jnp.linalg.norm(exact) + 1e-10
            )
    ...
    def plot_convergence(
        ...
        plt.text(
            0.05,
            0.95,
            f"Convergence rate: {conv_rate:.2f}",
    ...
    def run_comprehensive_tests(
        cls, derivative_orders: Optional[Tuple[int, ...]] = None
    ):
        """
