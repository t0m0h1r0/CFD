...
class SpatialDiscretizationTestSuite:
    ...
    def test_derivative_accuracy(
        ...
        im1 = ax1.pcolormesh(X[:,:,mid_idx], Y[:,:,mid_idx], numerical_deriv[:,:,mid_idx], 
                            shading='auto')
        ax1.set_title(f'Numerical {direction.upper()} Derivative')
        fig.colorbar(im1, ax=ax1)
        
        im2 = ax2.pcolormesh(X[:,:,mid_idx], Y[:,:,mid_idx], analytical_deriv[:,:,mid_idx], 
                            shading='auto')
        ax2.set_title(f'Analytical {direction.upper()} Derivative')
        fig.colorbar(im2, ax=ax2)
        
        error_map = jnp.abs(numerical_deriv - analytical_deriv)[:,:,mid_idx]
        im3 = ax3.pcolormesh(X[:,:,mid_idx], Y[:,:,mid_idx], error_map, 
                            shading='auto')
    ...
    def run_tests(cls):
        ...
        def dy_test_func1(x, y, z):
            """テスト関数1のy方向微分"""
            return jnp.pi * jnp.sin(jnp.pi * x) * jnp.cos(jnp.pi * y) * jnp.sin(jnp.pi * z)
        
        def dz_test_func1(x, y, z):
            """テスト関数1のz方向微分"""
            return jnp.pi * jnp.sin(jnp.pi * x) * jnp.sin(jnp.pi * y) * jnp.cos(jnp.pi * z)
