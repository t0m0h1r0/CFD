...
from src.core.linear_solvers.poisson_cg import (
    ConjugateGradientSolver as PoissonSolver,
    CGSolverConfig as PoissonSolverConfig,
)
...
class PoissonSolverTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_cases = [
            ...
            PoissonTestCase(
                ...
                rhs=lambda x, y, z: 2 * jnp.ones_like(x)
                + 2 * jnp.ones_like(y)
                + 2 * jnp.ones_like(z),
            ),
            PoissonTestCase(
    ...
    def setUp(self):
        ...
        self.laplacian = CCDLaplacianSolver(
            grid_manager=self.grid_manager, boundary_conditions=self.boundary_conditions
        )
    ...
    def test_convergence(self):
        ...
        for case in self.test_cases:
            ...
            numerical, history = self.solver.solve(
                self.laplacian, rhs, jnp.zeros_like(rhs)
            )
            ...
            # 結果の可視化
            self._plot_results(case.name, numerical, exact, error, history, diagnostics)
            ...
            # アサーション
            self.assertTrue(history["converged"], f"{case.name}: Failed to converge")
            self.assertLess(
                error,
                1e-2,  # 許容誤差
    ...
    def test_adaptive_tolerance(self):
        ...
        solver = PoissonSolver(config=config, grid_manager=self.grid_manager)
        ...
        self.assertTrue(
            history["converged"], "Solver should converge with adaptive tolerance"
        )
