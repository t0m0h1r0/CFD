...
class DiagonalPreconditioner:
    ...
    def apply(
        residual: ArrayLike, operator: CCDLaplacianSolver, scaling: float
    ) -> ArrayLike:
        """前処理を適用（改良版）"""
...
class ConjugateGradientSolver(LinearSolverBase):
    ...
    def __init__(
        ...
        self.preconditioner = (
            ...
            if config.preconditioner_type == "diagonal"
            else None
        )

    def enforce_boundary_conditions(
        self, field: ArrayLike, operator: CCDLaplacianSolver
    ) -> ArrayLike:
        """境界条件の適用（改良版）"""
    ...
    def _solve_iteration(
        ...
        # β の計算と安定化
        beta = jnp.where(rz > self.config.stabilization_threshold, rz_new / rz, 0.0)
        ...
        normalized_residual = residual_norm / (
            rhs_norm + self.config.stabilization_threshold
        )
