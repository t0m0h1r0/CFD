...
class PoissonSolverDiagnostics:
    ...
    def compute_diagnostic_metrics(
        solution: ArrayLike, operator: CCDLaplacianSolver, rhs: ArrayLike
    ) -> Dict[str, float]:
        """
        数値解の診断メトリクスを計算
...
class PoissonSolver(LinearSolverBase):
    ...
    def __init__(
        ...
        if discretization is None and grid_manager is not None:
            from src.core.spatial_discretization.operators.ccd_laplacian import (
                CCDLaplacianSolver,
            )
        ...
        for iteration in range(self.config.max_iterations):
            try:
                ...
                safe_residual = jnp.where(
                    jnp.isfinite(residual), residual, jnp.zeros_like(residual)
                )

                # 緩和付き更新
                field = field - (
                    self.config.relaxation_factor * safe_residual / diag_scale
                )

                # 相対残差の計算（数値安定性を考慮）
                residual_norm = jnp.linalg.norm(safe_residual) / (
                    jnp.linalg.norm(rhs) + self.config.stabilization_threshold
                )

                # 履歴の記録
                if self.config.record_history:
                    history.setdefault("residual_history", []).append(
                        float(residual_norm)
                    )

                # 収束判定
                if (
                    residual_norm < adaptive_tolerance
                    or jnp.isnan(residual_norm)
    ...
    def compute_diagnostics(
        self, solution: ArrayLike, operator: CCDLaplacianSolver, rhs: ArrayLike
    ) -> Dict[str, float]:
        """
        詳細な数値解析診断
    ...
    def adaptive_tolerance_adjustment(
        self, initial_tolerance: float, iteration_history: Dict
    ) -> float:
        """
        反復回数に基づく許容誤差の適応的調整
