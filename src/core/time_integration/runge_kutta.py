...
class RungeKutta4(TimeIntegratorBase):
    ...
    def compute_stage_derivatives(
        self, derivative_fn: Callable, field: ArrayLike, t: float
    ...
    def step(
        self, stage_derivatives: Tuple[ArrayLike, ...], t: float, field: ArrayLike
    ) -> ArrayLike:
        """
        GPU最適化されたステップ計算
...
class AdaptiveRungeKutta4(RungeKutta4):
    ...
    @partial(jax.jit, static_argnums=(0,))
    def adjust_timestep(self, error: ArrayLike, field: ArrayLike) -> ArrayLike:
        """
        時間ステップ幅の調整
        ...
        def reduce_dt():
            return jnp.maximum(
                jnp.array(0.1 * dt), jnp.array(0.9 * dt * (1 / error_ratio) ** (1 / 4))
            )

        def increase_dt():
            return jnp.minimum(
                jnp.array(10.0 * dt), jnp.array(0.9 * dt * (1 / error_ratio) ** (1 / 5))
            )

        return jax.lax.cond(error_ratio > 1.0, reduce_dt, increase_dt)
