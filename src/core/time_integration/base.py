...
class TimeIntegratorBase(ABC):
    ...
    @abstractmethod
    def step(self, dfield: ArrayLike, t: float, field: ArrayLike) -> ArrayLike:
        """
        1ステップの時間発展を実行
    ...
    def check_stability(
        self, dfield: ArrayLike, field: ArrayLike, t: float
    ) -> jnp.ndarray:
        """
        時間発展の安定性をチェック
        ...
        dt = self.config.dt
        stability_number = (
            dt * jnp.linalg.norm(dfield) / (jnp.linalg.norm(field) + 1e-10)
        )

        # JAX対応のため、booleanではなくfloat32で返す
        return jnp.array(
            stability_number <= self.config.safety_factor, dtype=jnp.float32
        )

    @staticmethod
    def vectorize_step(
        integrator: Callable, batch_dims: Union[int, Tuple[int, ...]] = 0
    ):
        """ベクトル化のためのラッパー"""
    ...
    def scan_vectorize_step(
        integrator: Callable, batch_initial_state: ArrayLike, time_steps: ArrayLike
    ):
        """scanによるベクトル化"""
