...
try:
    ...
    from preconditioner import list_preconditioners
    HAS_PRECOND = True
except ImportError:
    HAS_PRECOND = False
    precond_plugins = None
    list_preconditioners = lambda: []
...
def get_available_preconditioners() -> List[str]:
    ...
    try:
        # preconditionerモジュールの関数を利用
        return list_preconditioners()
...
def list_available_preconditioners() -> None:
    ...
    preconditioners = get_available_preconditioners()
...
def main():
    ...
    # 組み合わせ数をチェック
    total_combinations = len(function_list) * len(equation_list) * len(solver_list) * len(preconditioner_list)
    ...
    for equation_name in equation_list:
        for solver_name in solver_list:
            for preconditioner_name in preconditioner_list:
                ...
                print(f"\nセットアップ: {equation_name}, {solver_name}, {preconditioner_to_use or '前処理なし'}")
                            ...
                            try:
                                ...
                                if args.dim == 1:
                                    ...
                                    vis_file = visualizer.visualize_derivatives(
                                        ...
                                        prefix=f"{function_name}_{equation_name}_{solver_name}_{preconditioner_to_use or 'nopc'}"
                                    )
                                elif args.dim == 2:
    ...
    if len(all_results) > 1:
        ...
        print(f"  最短時間: {min_time['elapsed_time']:.4f}秒 ({min_time['function']}, {min_time['equation']}, {min_time['solver']}, {min_time['preconditioner'] or 'なし'})")
        print(f"  最長時間: {max_time['elapsed_time']:.4f}秒 ({max_time['function']}, {max_time['equation']}, {max_time['solver']}, {max_time['preconditioner'] or 'なし'})")
