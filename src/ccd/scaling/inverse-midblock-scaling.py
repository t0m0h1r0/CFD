...
class InverseMidBlockScaling(ScalingStrategy):
    ...
    def apply_scaling(self) -> Tuple[jnp.ndarray, Callable[[jnp.ndarray], jnp.ndarray]]:
        ...
        if n_total >= 2 * self.block_size:
            # 最初の内部ブロックを抽出
            B_actual = self.matrix[self.block_size:2*self.block_size, self.block_size:2*self.block_size]
        else:
        ...
        try:
            ...
            print("警告: 中央ブロックの逆行列計算に失敗しました。対角行列で代用します。")
        ...
        # スケーリング行列を保存
        self.scaling_matrix_row = jnp.kron(jnp.eye(n_blocks), B_inv)  # 行スケーリング行列
