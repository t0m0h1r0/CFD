...
class EquilibrationScaling(BaseScaling):
    ...
    def __init__(self, max_iterations=5, tolerance=1e-8, backend='numpy'):
        """
        反復パラメータを指定して初期化
    ...
    def scale(self, A, b) -> Tuple[Any, Any, Dict[str, Any]]:
        ...
        # スケーリングベクトルを初期化
        row_scale = self.array_utils.ones(m)
        col_scale = self.array_utils.ones(n)
        ...
        for _ in range(self.max_iterations):
            ...
            # 収束確認
            if (self.array_utils.abs(row_norms - 1.0) < self.tolerance).all() and \
               (self.array_utils.abs(col_norms - 1.0) < self.tolerance).all():
                break
            
            # スケーリング係数の更新（安定化のため平方根を使用）
            row_scale_update = 1.0 / self.array_utils.sqrt(self.array_utils.where(
                row_norms < 1e-15, 1.0, row_norms))
            col_scale_update = 1.0 / self.array_utils.sqrt(self.array_utils.where(
                col_norms < 1e-15, 1.0, col_norms))
            ...
            # 行列をスケーリング
            DR = self.array_utils.diags(row_scale_update)
            DC = self.array_utils.diags(col_scale_update)
    ...
    def _compute_row_norms(self, A, is_csr=False):
        ...
        if is_csr:
            ...
            for i in range(m):
                ...
                row_norms[i] = self.array_utils.max(self.array_utils.abs(row)) if row.size > 0 else 0.0
    ...
    def _compute_column_norms(self, A, is_csc=False):
        ...
        if is_csc:
            ...
            for j in range(n):
                ...
                col_norms[j] = self.array_utils.max(self.array_utils.abs(col)) if col.size > 0 else 0.0
