...
class ConditionMinimizer(BaseScaling):
    ...
    def scale(self, A: Union[sp.spmatrix, cp.ndarray], b: cp.ndarray) -> Tuple[Union[sp.spmatrix, cp.ndarray], cp.ndarray, Dict[str, Any]]:
        ...
        # まず平衡化スケーリングを基本として使用
        scaled_A, scaled_b, scale_info = EquilibrationScaling(max_iterations=2).scale(A, b)
        ...
        try:
            ...
            if initial_cond == float('inf') or cp.isnan(initial_cond):
            ...
            for power in self.power_range:
                try:
                    # 新しいスケーリング係数
                    new_row_scale = cp.power(row_scale, power)
                    new_col_scale = cp.power(col_scale, power)
                    ...
                    if test_cond < best_cond and test_cond > 0 and not cp.isnan(test_cond):
                        best_cond = test_cond
                        best_A = test_A
                        best_b = test_b
                        best_scale_info = {
    ...
    def _estimate_condition_number(self, A):
        ...
        try:
            ...
            if hasattr(A, 'toarray'):
                ...
                A_dense = A
            return float(cp.linalg.cond(A_dense))
        except:
            return float('inf')
