...
class SimilaritySymmetricScaling(ScalingStrategy):
    ...
    def get_param_info(cls) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "tol": {"type": float, "default": 1e-10, "help": "収束判定閾値"},
            "max_iter": {"type": int, "default": 10, "help": "最大反復回数"},
        }
    ...
    def apply_scaling(self) -> Tuple[jnp.ndarray, Callable[[jnp.ndarray], jnp.ndarray]]:
        ...
        if self.method == "equilibration":
            ...
            diag_elements = jnp.where(
                jnp.abs(diag_elements) < self.tol, self.tol, diag_elements
            )
            ...
            for i in range(n):
                ...
                if max_off_diag > jnp.abs(diag_elements[i]):
                    D_vals = D_vals.at[i].set(
                        jnp.sqrt(max_off_diag / jnp.abs(diag_elements[i]))
                    )
