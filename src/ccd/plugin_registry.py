...
class PluginRegistry(Generic[T]):
    ...
    def register(self, name: str, plugin_class: Type[T]) -> None:
        ...
        if not issubclass(plugin_class, self.base_class):
            raise TypeError(
                f"{plugin_class.__name__}は{self.base_class.__name__}のサブクラスではありません"
            )
    ...
    def scan_directory(self, directory: str) -> None:
        ...
        if not os.path.exists(directory) or not os.path.isdir(directory):
            self._log(
                f"警告: ディレクトリ {directory} が存在しないか、ディレクトリではありません"
            )
            return
                ...
                try:
                    ...
                    if spec and spec.loader:
                        ...
                        for class_name, obj in inspect.getmembers(
                            ...
                            if (
                                issubclass(obj, self.base_class)
                    ...
                    self._log(
                        f"警告: モジュール {module_name} のロード中にエラーが発生しました: {e}"
                    )
