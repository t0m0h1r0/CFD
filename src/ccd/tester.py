...
class CCDTester:
    ...
    def set_equation_set(self, equation_set_name):
        ...
        """
        # 統合EquationSetクラスを使用して、次元に基づいて適切なセットを作成
        dimension = 2 if self.is_2d else 1
    ...
    def get_test_function(self, func_name):
        ...
        """
        if self.is_2d:
            return self._get_2d_test_function(func_name)
        else:
            return self._get_1d_test_function(func_name)

    def _get_1d_test_function(self, func_name):
        """1Dテスト関数を取得"""
    ...
    def _get_2d_test_function(self, func_name):
        """2Dテスト関数を取得"""
        # まず基本的な2D関数を生成
    ...
    def _process_solution(self, test_func):
        """数値解と誤差を計算して結果を返す"""
        if self.is_2d:
            return self._process_2d_solution(test_func)
        else:
            return self._process_1d_solution(test_func)

    def _process_1d_solution(self, test_func):
        """1D解の処理"""
        # ソルバーで解を計算
    ...
    def _process_2d_solution(self, test_func):
        """2D解の処理"""
        # ソルバーで解を計算
    ...
    def run_grid_convergence_test(self, test_func, grid_sizes, x_range, y_range=None, use_dirichlet=True, use_neumann=True):
        ...
        for n in grid_sizes:
            ...
            if self.is_2d:
                ...
                # 両方向で同じ点数を使用
                grid = Grid(n, n, x_range=x_range, y_range=y_range)
            else:
