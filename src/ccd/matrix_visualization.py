...
def visualize_sparsity_pattern(matrix: sp.spmatrix, title: str = "Sparsity Pattern", 
                               figsize: Tuple[int, int] = (10, 10),
                               save_path: Optional[str] = None, dpi: int = 150) -> None:
    """
...
def visualize_matrix_blocks(matrix: sp.spmatrix, block_size: int, 
    ...
    vmin = np.min(block_density[block_density > 0]) if np.any(block_density > 0) else 1e-10
...
def visualize_matrix_values(matrix: sp.spmatrix, max_size: int = 1000, 
    ...
    if use_log:
        ...
        plt.imshow(pos_vals, cmap='Reds', norm=LogNorm(vmin=vmin, vmax=vmax), 
                  interpolation='none')
        
        # 負の値（青系）
        neg_vals = -np.ma.masked_where(~neg_mask, masked_matrix)  # 負の値を正に変換
        plt.imshow(neg_vals, cmap='Blues', norm=LogNorm(vmin=vmin, vmax=vmax), 
                  interpolation='none')
        ...
        plt.imshow(masked_matrix, cmap='RdBu_r', vmin=-vmax, vmax=vmax, 
                  interpolation='none')
...
def visualize_matrix_properties(matrix: sp.spmatrix, title: str = "Matrix Properties", 
    ...
    fig.suptitle(f"{title}\nSize: {matrix.shape}, NNZ: {matrix.nnz}, Density: {matrix.nnz / (matrix.shape[0] * matrix.shape[1]):.2e}", fontsize=16)
    ...
    plt.figtext(0.5, 0.01, diag_info, ha='center', fontsize=12, bbox={"facecolor": "lightgray", "alpha": 0.5, "pad": 5})
...
def analyze_and_visualize_matrix(matrix: sp.spmatrix, output_dir: str = "matrix_analysis", 
                                prefix: str = "matrix", dpi: int = 150) -> None:
    """
    ...
    if max(m, n) <= 2000:
        try:
            ...
            if m >= n and matrix.rank() == n:
                svd_vals = sp.linalg.svds(matrix, k=min(6, n-1), return_singular_vectors=False)
                cond_number = max(svd_vals) / min(svd_vals)
                print(f"条件数（推定）: {cond_number:.2e}")
                print(f"最大特異値: {max(svd_vals):.2e}, 最小特異値: {min(svd_vals):.2e}")
        except:
    ...
    # 2. ブロック構造の可視化
    block_path = os.path.join(output_dir, f"{prefix}_blocks_{matrix_size_str}_{block_size}.png")
    visualize_matrix_blocks(matrix, block_size=block_size, 
                           title=f"Matrix Block Structure (Block Size: {block_size})",
                           save_path=block_path, dpi=dpi)
