...
class CCDSolverDiagnostics:
    ...
    def analyze_boundary_conditions(self) -> Dict[str, Any]:
        ...
        if has_dirichlet:
            print(f"ディリクレ境界値: 左端={self.solver.grid_config.dirichlet_values[0]}, 右端={self.solver.grid_config.dirichlet_values[1]}")
        
        if has_neumann:
            print(f"ノイマン境界値: 左端={self.solver.grid_config.neumann_values[0]}, 右端={self.solver.grid_config.neumann_values[1]}")
        ...
        print("\n左端の境界条件に対応する左辺行列の行:")
        for i in left_idx:
            print(f"  行 {i}: {jnp.around(L_test[i, :8], decimals=3)}...")  # 最初の8要素のみ表示
        
        print("\n右端の境界条件に対応する左辺行列の行:")
        for i in right_idx:
            start_idx = max(0, i - 8)
            print(f"  行 {i}: ...{jnp.around(L_test[i, start_idx:i+1], decimals=3)}")
    ...
    def test_with_functions(self) -> Dict[str, Any]:
        ...
        for test_func in self.test_functions:
            ...
            if dirichlet_row_left >= 0:
                print(f"  右辺ベクトルでの値（左端、インデックス{dirichlet_row_left}）: {rhs[dirichlet_row_left]}")
            if dirichlet_row_right >= 0:
                print(f"  右辺ベクトルでの値（右端、インデックス{dirichlet_row_right}）: {rhs[dirichlet_row_right]}")
            ...
            if dirichlet_row_right >= 0:
                ...
                print(f"  ...{jnp.around(L_test[dirichlet_row_right, start_idx:dirichlet_row_right+1], decimals=3)}")
            
            # 係数の影響分析
            print(f"\n係数 [a={a}, b={b}, c={c}, d={d}] での右辺関数値:")
            print(f"  f = a*ψ + b*ψ' + c*ψ'' + d*ψ'''")
            print(f"  左端での右辺関数値: {a*f_values[0] + b*df_values[0] + c*d2f_values[0] + d*d3f_values[0]}")
            print(f"  右端での右辺関数値: {a*f_values[-1] + b*df_values[-1] + c*d2f_values[-1] + d*d3f_values[-1]}")
    ...
    def check_solution_values(self, func_name: str = "Sine") -> Dict[str, Any]:
        ...
        # テスト用の関数を選択
        test_func = next((f for f in self.test_functions if f.name == func_name), self.test_functions[0])
        ...
        print(f"ディリクレ境界値: 左端={dirichlet_values[0]}, 右端={dirichlet_values[1]}")
        ...
        print("\nディリクレ境界条件との一致:")
        print(f"  左端: {'一致' if left_match else '不一致'} (差={psi[0] - dirichlet_values[0]})")
        print(f"  右端: {'一致' if right_match else '不一致'} (差={psi[-1] - dirichlet_values[1]})")
    ...
    def perform_diagnosis(self, visualize: bool = False, test_func_name: str = "Sine") -> Dict[str, Any]:
        """
        基本的な診断を実行
        ...
        print(f"境界条件: {'ディリクレ' if has_dirichlet else ''}{'と' if has_dirichlet and has_neumann else ''}{'ノイマン' if has_neumann else ''}")
        if hasattr(self.solver, 'coeffs'):
