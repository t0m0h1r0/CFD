    ...
    def analyze_matrix_properties(self, visualize: bool = False) -> Dict[str, float]:
        ...
        if hasattr(self.solver, 'transformer') and hasattr(self.solver.transformer, 'L_scaled'):
            ...
            if visualize:
                ...
                visualize_matrix_properties(L_scaled, f"スケーリング後の行列特性 ({self.solver_name})", "results/scaled_matrix.png")
            
        # 元の行列を可視化
        if visualize:
            os.makedirs("results", exist_ok=True)
            visualize_matrix_properties(self.L, "元の行列特性", "results/original_matrix.png")
