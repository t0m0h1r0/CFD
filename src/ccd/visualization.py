...
class CCDVisualizer:
    ...
    def generate_filename(self, func_name: str, n_points: int, prefix: str = "") -> str:
        ...
        if prefix:
            return (
                f"{self.output_dir}/{prefix}_{func_name.lower()}_{n_points}_points.png"
            )
        else:
    ...
    def visualize_derivatives(
        ...
        for i, ax in enumerate(axes.flat):
            ...
            num_data = (
                numerical[i].get() if hasattr(numerical[i], "get") else numerical[i]
            )
    ...
    def compare_all_functions_errors(
        ...
        for i, (ax, error_type) in enumerate(zip(axes.flat, error_types)):
            ...
            if not has_zero:
                ...
                ax.text(
                    0.5,
                    0.95,
                    "Linear scale used due to zero values",
        ...
        # 保存
        filename = (
            f"{self.output_dir}/{prefix}_all_functions_comparison.png"
            if prefix
            else f"{self.output_dir}/all_functions_comparison.png"
        )
